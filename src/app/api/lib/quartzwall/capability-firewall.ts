...
function userAskedForDocument(userMessage: string) {
  return /\b(create|make|generate|write|export|download|save)\b[\s\S]{0,80}\b(document|file|pdf|docx|markdown|md|report|essay|notes)\b/i.test(
    userMessage
  );
}

function userAskedToOpenApp(userMessage: string) {
  return /\b(open|launch|play|start)\b/i.test(userMessage);
}
...
export function validateToolCall(
  ...
  if (toolName === "create_document") {
    ...
    if (!/^[\w .()-]{1,120}\.(pdf|docx|md)$/i.test(filename)) {
    ...
    if (!userAskedForDocument(userMessage)) {
      return result(false, 74, "Document creation requires explicit user intent", [
        signal("missing-user-intent", 74, "Document creation requires explicit user intent"),
      ]);
    }
