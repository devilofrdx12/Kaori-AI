...
export default function ModelSelector({
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div
          className={`absolute ${direction === "up" ? "bottom-full mb-2" : "top-full mt-2"} left-0 w-[min(18rem,calc(100vw-2rem))] max-h-[50vh] rounded-2xl border border-white/70 dark:border-white/10 bg-white/90 dark:bg-neutral-950/92 backdrop-blur-md backdrop-saturate-150 shadow-2xl z-50 overflow-y-auto overflow-x-hidden animate-fade-in`}
        >
          {MODEL_OPTIONS.map((m) => (
            <button
              key={m.id}
